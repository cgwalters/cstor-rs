...
//! # Layer Hierarchy
//!
//! Layers form a chain from the current layer down to the base:
//! - Each layer may have one or more parent layers
//! - Parents are referenced by short link IDs in the `lower` file
//! - The base layer has no `lower` file
//! - Changes overlay on top of parent layers
//!
...
//!
//! These markers ensure proper layering semantics when files are deleted
//! or directories are replaced in upper layers.
//!
//! # Usage Example
//!
//! ```no_run
...
//! println!("Parents: {:?}", layer.parent_links());
//!
//! // Access files in the layer
//! let file = layer.open_file("etc/hosts")?;
//!
//! // Check for whiteouts
//! if layer.has_whiteout("etc", "shadow")? {
//!     println!("etc/shadow is whited out");
//! }
//!
//! // Get full layer chain (consumes layer)
//! let chain = layer.layer_chain(&storage)?;
...
//!
//! All file access is performed through the `diff_dir` [`cap_std::fs::Dir`] handle,
//! ensuring operations cannot escape the layer directory. This provides strong
//! security guarantees against path traversal attacks.
...
impl Layer {
    ...
    ///
    /// Unlike [`parent_links()`](Self::parent_links) which returns the raw short
    /// link IDs, this method returns the actual 64-character layer digests.
    ///
    /// # Errors
    ///
    ...
    ///
    /// This provides direct access to the layer directory (`overlay/<layer-id>/`)
    /// for advanced use cases like reading the `link` or `lower` files directly.
    /// For accessing layer content, prefer using [`open_file()`](Self::open_file)
    /// which operates on the `diff/` subdirectory.
